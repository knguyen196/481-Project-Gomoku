    ...
    for (let col = 0; col < SIZE; col++) {
      for (let p = 0; p < 2; p++) {
        ...
        for (let d = 0; d < directions.length; d++) {
          ...
          count = countLine(row, col, dx, dy, player);
          if (count >= 1) {
              if (row - dx < 0 || row + dx * count >= SIZE || col - dy < 0 || col + dy * count >= SIZE) {
                  count -= 1;
              }
              else if(board[row - dx][col - dy] === otherPlayer || board[row + dx * count][col + dy * count] === otherPlayer) {
                  count -= 1;
              }
          }
